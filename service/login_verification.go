...
type loginFlowPayload struct {
	...
	PendingGitHubCreatedAt int64  `json:"pending_github_created_at,omitempty"`
}
...
func StartLoginVerification(user *model.User, loginMethod string, migration *LegacyGitHubMigration) (*LoginChallenge, error) {
	...
	if migration != nil {
		payload.PendingGitHubID = migration.GitHubID
		payload.PendingGitHubLegacyID = migration.LegacyID
		payload.PendingGitHubCreatedAt = migration.CreatedAt
	}
...
func CompleteLoginVerification(token string, verification *LoginVerification, method, ip, userAgent string) (*AuthBundle, *LegacyGitHubMigration, error) {
	...
	if err := model.CreateUserSessionFromLoginFlow(token, session, func(tx *gorm.DB, flow *model.AuthFlow, state *model.UserVerificationState) error {
		...
		if written {
			if payload.PendingGitHubCreatedAt > 0 {
				if err := tx.Model(&model.User{}).
					Where("id = ? AND github_id = ?", state.UserID, payload.PendingGitHubID).
					Update("github_created_at", payload.PendingGitHubCreatedAt).Error; err != nil {
					return err
				}
			}
			migration = &LegacyGitHubMigration{
				GitHubID:  payload.PendingGitHubID,
				LegacyID:  payload.PendingGitHubLegacyID,
				CreatedAt: payload.PendingGitHubCreatedAt,
			}
		}
		return nil
	}); err != nil {
