...
func ClassifyRelayOutcome(ctx context.Context, info *relaycommon.RelayInfo, apiErr *types.NewAPIError) Outcome {
	...
	if apiErr != nil {
		if perf_metrics_setting.GetSetting().ExcludeUpstream400Errors && apiErr.GetUpstreamStatusCode() == 400 {
			return OutcomeIgnored
		}
...
func classifyFailure(local bool, code, errorType string, status int) Outcome {
	...
	case 405, 409, 413, 415, 422:
		return OutcomeIgnored
	case 400:
		if perf_metrics_setting.GetSetting().ExcludeUpstream400Errors {
			return OutcomeIgnored
		}
	}
	return OutcomeFailure
