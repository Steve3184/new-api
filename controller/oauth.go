	...
	if strings.ContainsFunc(legacyID, func(r rune) bool { return r < '0' || r > '9' }) && provider.IsUserIDTaken(legacyID) {
		...
		if user.Id != 0 {
			...
			if state.HasTwoFA || state.HasPasskey {
				...
				return user, &service.LegacyGitHubMigration{
					GitHubID:  oauthUser.ProviderUserID,
					LegacyID:  legacyID,
					CreatedAt: oauthUser.RegisteredAt,
				}, nil
			}
			...
			err = model.DB.Transaction(func(tx *gorm.DB) error {
				...
				if err != nil || !written || oauthUser.RegisteredAt <= 0 {
					return err
				}
				return tx.Model(&model.User{}).
					Where("id = ? AND github_id = ?", user.Id, oauthUser.ProviderUserID).
					Update("github_created_at", oauthUser.RegisteredAt).Error
			})
			if err != nil {
				return nil, nil, err
			}
