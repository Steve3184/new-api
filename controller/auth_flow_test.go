...
func TestFindOrCreateOAuthUserPersistsAffiliateInviter(t *testing.T) {
	...
	}, &oauth.OAuthToken{}, inviter.AffCode)
	require.NoError(t, err)
...
func (provider *legacyGitHubOAuthProvider) GetUserInfo(context.Context, *oauth.OAuthToken) (*oauth.OAuthUser, error) {
	return &oauth.OAuthUser{
		...
		Username:       provider.legacyID,
		RegisteredAt:   provider.registeredAt,
		Extra:          map[string]any{"legacy_id": provider.legacyID},
	}, nil
}
...
func TestOAuthLoginLegacyGitHubBindingRequiresAccountEvidence(t *testing.T) {
	const registeredAt int64 = 1_600_000_000
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			provider := &legacyGitHubOAuthProvider{providerUserID: "900001", legacyID: test.legacyID, registeredAt: registeredAt, verifiedEmails: test.verifiedEmails, verifiedEmailsErr: test.verifiedEmailsErr}
			...
			if test.expectMigration {
				...
				assert.Equal(t, registeredAt, reloaded.GitHubCreatedAt)
				assert.NotEmpty(t, response.Header().Values("Set-Cookie"))
				return
			}
...
func TestOAuthLoginLegacyGitHubBindingMigratesAfterLoginVerification(t *testing.T) {
	...
	for _, interference := range []string{"none", "pending id claimed by another account", "binding relinked meanwhile"} {
		t.Run(interference, func(t *testing.T) {
			...
			require.NoError(t, model.DB.Create(factor).Error)
			provider := &legacyGitHubOAuthProvider{providerUserID: "900001", legacyID: "octocat-legacy", registeredAt: registeredAt, verifiedEmails: []string{"legacy-github@example.com"}}
			response := legacyGitHubOAuthLogin(t, provider)
			...
			if interference == "none" {
				...
				assert.Zero(t, reloaded.GitHubCreatedAt)
			}
