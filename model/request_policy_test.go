...
func TestRequestPolicyDatabaseMatrix(t *testing.T) {
	for _, dialect := range []string{"sqlite", "mysql", "postgres"} {
		t.Run(dialect, func(t *testing.T) {
			...
			require.NoError(t, UpdateRequestPolicyOptions(map[string]string{"RetryTimes": "2", "ChannelAutoStatusEmailEnabled": "false", "channel_affinity_setting.rules": rules}))
			...
			assert.False(t, common.ChannelAutoStatusEmailEnabled, "the request policy API preserves the fork notification setting")
